...
public final class EditingContextServices {
    ...
    public EObject findObjectMatching(IEditingContext editingContext, String featureName, String value) {
        //@formatter:off
        return this.getResourceset(editingContext)
                .stream()
                .flatMap(this::collectAllContent)
                .filter(eObject -> this.hasFeatureMatchingNameAndValue(eObject, featureName, value))
                .findFirst()
                .orElse(null);
         //@formatter:on
    }

    private boolean hasFeatureMatchingNameAndValue(EObject eObject, String featureName, String value) {
        Optional<Object> optionalValue = this.getFeatureValueWithName(eObject, featureName);
        return optionalValue.filter(featureValue -> value.equals(String.valueOf(featureValue))).isPresent();
    }

    private Optional<Object> getFeatureValueWithName(EObject eObject, String featureName) {
        //@formatter:off
