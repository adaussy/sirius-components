 ...
 */
@Service
public class ProjectImportService implements IProjectImportApplicationService {
    ...
    public ProjectImportService(ImportProjectContentBuilder importProjectContentBuilder, IProjectCreationService projectCreationService, IProjectMapper projectMapper) {
    ...
    public IPayload importProject(UploadProjectInput input) {
        ...
        try {
            ProjectImportContent projectStructure = importProjectContentBuilder.buildFromZip(input.file().getInputStream());

            UploadProjectContentInput uploadContentInput = new UploadProjectContentInput(input.id(), input, projectStructure);

            IResult<Project> result = this.projectCreationService.createProject(uploadContentInput, projectStructure.getName(), projectStructure.getNatures());
            ...
            payload = new ErrorPayload(input.id(), "Unable to import project: " + e.getMessage());
        }
